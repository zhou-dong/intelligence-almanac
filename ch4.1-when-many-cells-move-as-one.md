# Facts — When Many Cells Move as One (Chapter 4.1)

*This file holds facts specific to this chapter. This is sub-chapter 4.1 of Chapter 4 — same period (Ediacaran) and same organism (a generic early bilaterian, not a specific named species) as sub-chapter 4.2 (`ch4.2-when-weights-learn.md`), split out only to keep each file a manageable length. 4.1 covers structure/convergence (fixed weights); 4.2 covers weights actually changing from experience. Renumbered from ch2.1/ch2.2 to ch4.1/ch4.2 on 2026-09-21 to make room for standalone sponges (`ch2-when-there-is-no-one-home.md`) and cnidarians (`ch3-when-the-first-synapse-fires.md`) chapters.*
...
## Cnidarian nervous system — now its own chapter

*This section previously held the cnidarian background material now expanded into its own standalone chapter, `ch3-when-the-first-synapse-fires.md`, once cnidarians got their own chapter number (2026-09-21). The one fact worth restating here, since it directly bears on this chapter's claims:*

- The *tunable synapse* (a weight sitting in a connection, adjustable independently of the sensor) predates bilaterians — it belongs to cnidarians (`ch3-when-the-first-synapse-fires.md`), not to this chapter. What bilaterians add is **centralization** — a specific place where signals from across the body converge into one decision — not the synapse itself. (established; see ch3 for the full nerve-net/habituation evidence)
...
- **Structure vs. training — don't conflate these:** having the convergence structure (a ganglion) is not the same as being able to train it with Rosenblatt's error-driven rule. No bilaterian, including the best-documented living case, has been shown to implement literal error/target comparison — real documented plasticity (e.g. associative learning in *Aplysia*, per `ch4.2-when-weights-learn.md`) is mechanistically simpler than that. So "has the structure" and "trains like a perceptron" are separate claims with different evidence behind them. (established distinction between the structure and the specific learning rule)
...
- Genuine individual-lifetime learning about food and danger — associating a specific stimulus with an outcome and changing behavior because of it (classical/operant conditioning) — is documented in some invertebrates (e.g., Aplysia, various insects), but requires outcome-comparing machinery closer to ch4.2's error-driven perceptron rule (`ch4.2-when-weights-learn.md § The perceptron learning rule`) than to anything in this chapter. Attributing that capability to this chapter's organism specifically would be a stronger, less defensible claim than the fixed-reflex framing above. (plausible caution against overclaiming; the perceptron rule itself is established, per ch4.2)
...
- **Branching order among animal lineages is a tree, not a ladder.** Sponges, cnidarians, and bilaterians are not sequential steps where one evolves into the next — each split leaves two sister lineages, only one of which keeps splitting further. The traditional (though actively disputed) view: the early animal ancestor's lineage split off sponges first, then the remaining lineage split off cnidarians, leaving bilaterians as the last-splitting group — meaning sponges and cnidarians are bilaterians' cousins, still alive today, not ancestors bilaterians "passed through." A competing hypothesis places ctenophores (comb jellies) as the first branch instead of sponges; this remains unresolved. (plausible — branching order specifically is an active, unresolved debate; the tree-not-ladder structure itself is established)
  ...
  **Full (annotated — where learning does and doesn't appear):**
  ...
  ```
- **Cnidarians are the earliest-branching lineage where synapse-weight-updating-from-experience is documented** (habituation), and convergence/centralization is this chapter's own, later contribution — the two capabilities are independent, and plasticity is the older one. Full evidence, mechanism, and the *Aplysia* contrast now live in `ch3-when-the-first-synapse-fires.md § Experience-based plasticity — habituation`, not duplicated here. (established for cnidarians; see ch3)
